...
(function($, window, document, undefined) {
	...
	var MyDatagrid = function(oInit) {
		 ...
		 * * @param {object} rowData 双击事件是双击选中的行数据
		 */
		function _dataHandlePo(oGrid, handler, bDetail, rowData) {
			var selectRow = rowData;
			var bMany = false;
			if(!selectRow){
				var dataGrid = $(oGrid);
			...
			if (handler.queryUrl) { // URL存在，则进行后台查询
				$.ajax({
					...
					"success" : function(data) { // 返回需要修改的数据信息
						if (data) {
							_dataFormHandlePo(oGrid, handler, data, bDetail, bMany);
						}
					},
					"error" : function() {
						$.messager.alert("提示", "AJAX请求失败，请检查URL：" + handler.queryUrl);
				...
				_dataFormHandlePo(oGrid, handler, selectRow, bDetail, bMany);
			}
		}
		/**
		 * @summary 数据处理器 针对页面带时间 time
		 * @param {object} oGrid datagrid
		 * @param {object} dataHandle 处理器配置信息
		 * @param {boolean} bDetail 查看数据开关
		 * * @param {object} rowData 双击事件是双击选中的行数据
		 */
		function _dataHandleTime(oGrid, handler, bDetail, rowData) {
			var selectRow = rowData;
			var bMany = false;
			if(!selectRow){
				var dataGrid = $(oGrid);
...
//				页面带有时间格式的处理  billingDate
//				var inputDate = new Date(selectRows[0].billingDate);
...
//				selectRows[0].billingDate = inputDate;
				selectRow = selectRows[0]; // 默认对选中的第一条数据进行操作
				if (selectRows.length > 1) {
					bMany = true; // 用户选中多条数据
					var firstIndex = dataGrid.datagrid("getRowIndex", selectRow);
			...
			if (handler.queryUrl) { // URL存在，则进行后台查询
				$.ajax({
					...
					"success" : function(data) { // 返回需要修改的数据信息
						if (data) {
							var da = data;
							_dataFormHandleTime(oGrid, handler, data, bDetail, bMany);
						}
					},
					"error" : function() {
						$.messager.alert("提示", "AJAX请求失败，请检查URL：" + handler.queryUrl);
				...
				_dataFormHandleTime(oGrid, handler, selectRow, bDetail, bMany);
			}
		}
		/**
		 * @summary 数据处理器
...
//			dh = dh>wh?wh:dh;
		 ...
		 * @param {boolean} bMany 多条数据开关
		 */
		function _dataFormHandlePo(oGrid, handler, formData, bDetail, bMany) {
			var dataForm = $("#" + handler.form.id);
			if (!dataForm || dataForm.length <= 0) {
				$.messager.alert("提示", "ID为：" + handler.form.id + " 的表单不存在！");
				return;
...
//			alert(formData);
			...
			formData.redFlag2 = formData.redFlag;
...
//			dh = dh>wh?wh:dh;
		 ...
		 * @param {boolean} bMany 多条数据开关
		 */
		function _dataFormHandleTime(oGrid, handler, formData, bDetail, bMany) {
			var dataForm = $("#" + handler.form.id);
			if (!dataForm || dataForm.length <= 0) {
				$.messager.alert("提示", "ID为：" + handler.form.id + " 的表单不存在！");
				return;
...
//			dh = dh>wh?wh:dh;
		...
		return this.each(function() {
			...
			//按钮权限控制
			var _toolbars = [];
			//alert("oInit.toolbar:::"+oInit.toolbar);
			if(oInit.toolbar){
				
				$.each(oInit.toolbar, function(ei, en){
					//alert("permissable(en.permission):::"+permissable(en.permission));
			...
			/* 编辑器处理 me */
			if (oInit.editHandlerPo && oInit.editHandlerPo.enable) {
				oInit.editHandlerPo = _defaultParams(oInit.model, "edit", oInit.editHandlerPo, oInit.system,  false);
				if(permissable(oInit.editHandlerPo.permission)){
					var editBtn = [ {
						"id" : "myg_btnedit",
						"text" : oInit.editHandlerPo.title,
						"iconCls" : "icon-edit",
						"handler" : function() {
							_dataHandlePo(_this, oInit.editHandlerPo);
						}
					} ];
					oInit.toolbar = $.merge(editBtn, oInit.toolbar);
				}
			...
			/* 编辑器处理  针对时间*/
			if (oInit.editHandlerTime && oInit.editHandlerTime.enable) {
				oInit.editHandlerTime = _defaultParams(oInit.model, "edit", oInit.editHandlerTime, oInit.system,  false);//
				if(permissable(oInit.editHandlerTime.permission)){
					var editBtn = [ {
						"id" : "myg_btnedit",
						"text" : oInit.editHandlerTime.title,
						"iconCls" : "icon-edit",
						"handler" : function() {
							_dataHandleTime(_this, oInit.editHandlerTime);
						}
					} ];
					oInit.toolbar = $.merge(editBtn, oInit.toolbar);
				}
	...
	$.extend($.fn.validatebox.defaults.rules, {
		...
		hedgeCurrency : {// 验证对冲货币 首位为“-”号
			validator : function(value) {
				return /^[-]\d+(\.\d+)?$/i.test($.trim(value));
			},
			message : '红字标识的货币格式不正确'
		},
		qq : {// 验证QQ,从10000开始
			validator : function(value) {
...
var GM_PERMISSIONS = {
	"enable": false,
	"url": "/mdm-web/mng/loginPermission.json",
...
function permissable(permission){
	...
	//alert("GM_PERMISSIONS.enable:"+GM_PERMISSIONS.enable);
